					 ...
					 enum StrWorkAPI api, bool finished)
{
	...
	KUNIT_EXPECT_EQ(test, r->total + r->remaining, r->initial);
...
static void work_match_test(struct kunit *test)
{
	...
	for (n = 2; n <= 8; n *= 2) {
		...
		r = work_record(test, 0, SW_MATCH, true);
		/* 2^n entries, 2^(n+1)-1 dispatches and pattern scans,
		 * one fewer subject check, and 2*(2^n-1) retries.
		 */
		KUNIT_EXPECT_EQ(test, r->total, 9ULL * (1ULL << n) - 4);
		...
		work_run(test, chunk, 0);
		r = work_record(test, 0, SW_MATCH, true);
		KUNIT_EXPECT_EQ(test, r->total, 6ULL * n + 9);
	}
	for (n = 4; n <= 16; n *= 2) {
		memset(src, 'a', n);
		src[n] = '\0';
		...
		work_run(test, chunk, 0);
		/* Each full attempt costs start+entry+4*(dispatch+pattern+subject).
		 * The final four suffixes cost 13, 10, 7 and 4; add operation=1.
		 */
		KUNIT_EXPECT_EQ(test, work_record(test, 0, SW_MATCH, true)->total, 14ULL * n - 7);
		for (kind = 0; kind < 2; kind++) {
			snprintf(chunk, sizeof(chunk), "assert(string.match('%s', '%s') == nil)",
				 src, kind ? "^a-a-a-b" : "^a*a*a*b");
			work_run(test, chunk, 0);
			r = work_record(test, 0, SW_MATCH, true);
			KUNIT_EXPECT_GT(test, r->total, previous[kind]);

			previous[kind] = r->total;
		}
	}
	kunit_info(test, "literal match=9; optional/greedy/minimal growth checked\n");
}
...
static void work_scan_test(struct kunit *test)
{
	...
	for (c = 8; c <= 64; c *= 2) {
		...
		work_run(test, chunk, 0);
		r = work_record(test, 0, SW_MATCH, true);
		/* 17 pattern scans, 16 class scans, 51 entry/dispatch/start
		 * units, 16 subject checks and one operation unit.
		 */
		KUNIT_EXPECT_EQ(test, r->total, 33ULL * (c + 2) + 68);

		snprintf(chunk, sizeof(chunk), "assert(string.match('xxxxxxxxxxxxxxxx', '%%f[%s]z') == nil)", members);
		work_run(test, chunk, 0);
		r = work_record(test, 0, SW_MATCH, true);
		KUNIT_EXPECT_EQ(test, r->total, 51ULL * (c + 2) + 86);
	}
	work_run(test, "assert(string.match('b', '[%%a-cx]') == 'b')", 0);

	work_run(test, "assert(string.match('aa', '^%f[a]', 2) == nil)", 0);
	work_record(test, 0, SW_MATCH, true);
	for (n = 4; n <= 16; n *= 2) {
		memset(src, '(', n + 1);
		memset(src + n + 1, ')', n + 1);
		src[2 * n + 2] = '\0';
		...
		work_run(test, chunk, 0);
		r = work_record(test, 0, SW_MATCH, true);
		KUNIT_EXPECT_EQ(test, r->total, 4ULL * n + 10);

		src[2 * n + 1] = '\0';
		snprintf(chunk, sizeof(chunk), "assert(string.match('%s', '^%%b()$') == nil)", src);
		work_run(test, chunk, 0);
		KUNIT_EXPECT_EQ(test, work_record(test, 0, SW_MATCH, true)->total, 2ULL * n + 5);
	}
}
...
static void work_capture_test(struct kunit *test)
{
	...
	for (n = 8; n <= 32; n *= 2) {
		...
		work_run(test, chunk, 0);
		r = work_record(test, 0, SW_MATCH, true);
		/* n retries close the capture; half compare and try the trailing b. */
...
static void work_output_test(struct kunit *test)
{
	...
	for (ref = 0; ref <= 1; ref++) {
		for (repeats = 1; repeats <= 2; repeats++) {
			...
			/* Four matches plus the final failed attempt; capture
			 * matching adds 22, each %0/%1 append adds 3/5.
			 */
			KUNIT_EXPECT_EQ(test, r->total,
				       33ULL + 22 * ref + (12 + 8 * ref) * repeats);
		}
	}
...
static void work_recovery_test(struct kunit *test)
{
	...
	work_record(test, 0, SW_MATCH, false);
	...
	work_record(test, 0, SW_GSUB, false);
...
static void work_integer_test(struct kunit *test)
{
	...
	/* No sticky state: production unwinds immediately after a rejection. */
	KUNIT_EXPECT_TRUE(test, strwork_debit(&b, 0));
...
static struct lua_work_record *work_rejected(struct kunit *test, unsigned int i)
{
	...
	KUNIT_EXPECT_EQ(test, r->total + r->rejected_remaining, r->initial);
 ...
 * Lua assertions independently check successful results; existing observation cases
 * retain the independent total/formula oracles for the full operations.
 */
static void work_limit_sweep_test(struct kunit *test)
{
	...
	kunit_info(test, "work-limit sweep: %u vectors, %u calls\n",
		   v, attempts);
}
...
static void work_limit_iterator_test(struct kunit *test)
{
	...
	lua_pushvalue(L, 1);
	KUNIT_ASSERT_EQ(test, lua_pcall(L, 0, LUA_MULTRET, 0), LUA_ERRRUN);
	KUNIT_EXPECT_EQ(test, work_rejected(test, 0)->total, 323ULL);
	KUNIT_ASSERT_EQ(test, lua_gettop(L), 2);
	lua_settop(L, 1);
	work_limit(f, 324);
	lua_pushvalue(L, 1);
...
static void work_limit_buffer_test(struct kunit *test)
{
	...
	for (i = 0; i < 3; i++) {
		...
		work_run(test, chunk, LUA_ERRRUN);
		r = work_rejected(test, 0);
		/* Two units per replacement byte: scan and append. */
		KUNIT_EXPECT_GT(test, r->total, 2ULL * LUAL_BUFFERSIZE);
...
static void work_limit_observer_test(struct kunit *test)
{
	...
	/* Lost observation slots neither reject affordable work nor waive limits. */
	lua_settop(L, 0);
	KUNIT_ASSERT_EQ(test, luaL_loadbuffer(L, small, strlen(small), "full-success"), 0);
	KUNIT_ASSERT_EQ(test, lua_pcall(L, 0, 1, 0), 0);
	KUNIT_EXPECT_STREQ(test, lua_tostring(L, -1), "a");
...
MODULE_DESCRIPTION("KUnit Lua string budget properties");
MODULE_LICENSE("GPL");
