...
static int matchbracketclass (MatchState *ms, int c, const char *p,
                             const char *ec) {
  int sig = 1;
  strwork_charge(ms->work, 1);  /* opening bracket */
  if (*(p+1) == '^') {
    strwork_charge(ms->work, 1);
    sig = 0;
    p++;  /* skip the `^' */
  ...
  while (++p < ec) {
    if (*p == L_ESC) {
      strwork_charge(ms->work, 2);
      p++;
      if (match_class(c, uchar(*p)))
        return sig;
    }
    else if ((*(p+1) == '-') && (p+2 < ec)) {
      strwork_charge(ms->work, 3);
      p+=2;
  ...
  strwork_charge(ms->work, 1);  /* closing bracket on exhaustion */
  return !sig;
}
...
static const char *find_first (StrWork *work, const char *s, int c, size_t len) {
  ...
  */
  strwork_charge(work, found ? (size_t)(found-s) + 1 : allowed);
  if (found) return found;
  /* Preserve the one-byte rejection before scanning beyond the allowance. */
  if (allowed < len) strwork_charge(work, 1);
  return NULL;
}
...
static int str_gsub (lua_State *L) {
  ...
  strwork_charge(ms.work, ms.src_end-src);
