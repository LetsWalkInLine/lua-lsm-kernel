...
#define strwork_ms(ms, cost) strwork_charge((ms)->work, cost)
...
static int matchbracketclass (MatchState *ms, int c, const char *p,
                             const char *ec) {
  int sig = 1;
  strwork_ms(ms, 1);  /* opening bracket */
  if (*(p+1) == '^') {
    strwork_ms(ms, 1);
    sig = 0;
    p++;  /* skip the `^' */
  ...
  strwork_ms(ms, 1);  /* closing bracket on exhaustion */
  return !sig;
}
