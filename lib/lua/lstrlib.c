...
/* Root plus the opening and closing of 32 captures.  This does not
** bound the combined stack of nested Lua or string library calls.
*/
#ifndef LUA_PATTERN_MAXDEPTH
#define LUA_PATTERN_MAXDEPTH 65
#endif
...
static int capture_to_close (MatchState *ms) {
  ...
  for (level--; level>=0; level--) {
    strwork_ms(ms, SW_CAPTURE_SLOT, 1);
...
static int matchbracketclass (MatchState *ms, int c, const char *p,
                             const char *ec) {
  int sig = 1;
  strwork_ms(ms, SW_CLASS, 1);  /* opening bracket */
  if (*(p+1) == '^') {
    strwork_ms(ms, SW_CLASS, 1);
    sig = 0;
    p++;  /* skip the `^' */
  }
  while (++p < ec) {
    if (*p == L_ESC) {
      strwork_ms(ms, SW_CLASS, 2);
      p++;
      if (match_class(c, uchar(*p)))
        return sig;
    }
    else if ((*(p+1) == '-') && (p+2 < ec)) {
      strwork_ms(ms, SW_CLASS, 3);
      p+=2;
      ...
      strwork_ms(ms, SW_CLASS, 1);
      if (uchar(*p) == c) return sig;
    }
  }
  strwork_ms(ms, SW_CLASS, 1);  /* closing bracket on exhaustion */
  return !sig;
}


static int singlematch (MatchState *ms, int c, const char *p, const char *ep) {
  strwork_ms(ms, SW_SUBJECT, 1);
  switch (*p) {
...
static const char *max_expand (MatchState *ms, const char *s,
  ...
  while ((s+i)<ms->src_end && singlematch(ms, uchar(*(s+i)), p, ep))
    i++;
  /* keeps trying to match with the maximum repetitions */
  while (i>=0) {
    const char *res;
    strwork_ms(ms, SW_RETRY, 1);
...
static const char *min_expand (MatchState *ms, const char *s,
  ...
  for (;;) {
    ...
    if (res != NULL)
      return res;
    else if (s<ms->src_end && singlematch(ms, uchar(*s), p, ep))
...
static const char *match_capture (MatchState *ms, const char *s, int l) {
  ...
  if ((size_t)(ms->src_end-s) >= len) {
    /* Charge the requested extent, independently of memcmp's early exit. */
    strwork_ms(ms, SW_COMPARE, len);
...
static const char *match (MatchState *ms, const char *s, const char *p) {
  strwork_ms(ms, SW_ENTER, 1);
  if (ms->matchdepth == 0)
    luaL_error(ms->L, "pattern recursion limit exceeded");
  ms->matchdepth--;
  init: /* using goto's to optimize tail recursion */
  strwork_ms(ms, SW_DISPATCH, 1);
  switch (*p) {
    case '(': {  /* start capture */
    ...
    case L_ESC: {
      switch (*(p+1)) {
        ...
        case 'f': {  /* frontier? */
          ...
          strwork_ms(ms, SW_SUBJECT, 1);
          if (!matchbracketclass(ms, uchar(*s), p, ep-1)) {
            s = NULL;
            goto out;
          }
    ...
    default: dflt: {  /* it is a pattern item */
      ...
      int m = s<ms->src_end && singlematch(ms, uchar(*s), p, ep);
      switch (*ep) {
        case '?': {  /* optional */
          const char *res;
          if (m) {
...
/* Keep C-NUL classification separate from length-aware plain searching. */
static int has_specials (StrWork *work, const char *p) {
  for (;;) {
    strwork_charge(work, SW_CLASSIFY, 1);
    if (*p == '\0') return 0;
    if (strchr(SPECIALS, *p)) return 1;
    p++;
  }
}
...
static const char *lmemfind (StrWork *work, const char *s1, size_t l1,
                               const char *s2, size_t l2) {
  if (l2 == 0) return s1;  /* empty strings are everywhere */
  else if (l2 > l1) return NULL;  /* avoids a negative `l1' */
  else {
    const char *init;  /* to search for a `*s2' inside `s1' */
    l2--;  /* 1st char will be checked by `find_first' */
    l1 = l1-l2;  /* `s2' cannot be found after that */
    while (l1 > 0 && (init = find_first(work, s1, *s2, l1)) != NULL) {
      strwork_charge(work, SW_PLAIN_CANDIDATE, 1);
      init++;   /* 1st char is already checked */
      strwork_charge(work, SW_COMPARE, l2);
...
static void push_onecapture (MatchState *ms, int i, const char *s,
                                                    const char *e) {
  strwork_ms(ms, SW_CAPTURE_EMIT, 1);
  if (i >= ms->level) {
    if (i == 0) {  /* ms->level == 0, too */
      strwork_ms(ms, SW_OUTPUT, e - s);
...
static int str_find_aux (lua_State *L, int find) {
  ...
  strwork_begin(L, &work, find ? SW_FIND : SW_MATCH);
  if (find && (lua_toboolean(L, 4) ||  /* explicit request? */
      !has_specials(&work, p))) {  /* or no special characters? */
    /* do a plain search */
    const char *s2 = lmemfind(&work, s+init, l1-init, p, l2);
    if (s2) {
    ...
    do {
      const char *res;
      ms.level = 0;
      strwork_charge(&work, SW_START, 1);
      if ((res=match(&ms, s1, p)) != NULL) {
        int n;
        if (find) {
          lua_pushinteger(L, s1-s+1);  /* start */
          lua_pushinteger(L, res-s);   /* end */
          n = push_captures(&ms, NULL, 0) + 2;
        }
        else
          n = push_captures(&ms, s1, res);
        strwork_end(&work);
        return n;
      }
...
static int gmatch_aux (lua_State *L) {
  ...
  const char *src;
  strwork_begin(L, &work, SW_GMATCH);
  ms.L = L;
  strwork_bind(&ms, &work);
  ms.matchdepth = LUA_PATTERN_MAXDEPTH;
  ms.src_init = s;
...
static void add_s (MatchState *ms, luaL_Buffer *b, const char *s,
  ...
  for (i = 0; i < l; i++) {
    strwork_ms(ms, SW_REPLACEMENT_SCAN, 1);
    if (news[i] != L_ESC) {
      strwork_ms(ms, SW_OUTPUT, 1);
      ...
      if (!isdigit(uchar(news[i]))) {
        ...
        add_work_value(ms, b);  /* add capture to accumulated result */
      }
    }
  }
}
...
static void add_value (MatchState *ms, luaL_Buffer *b, const char *s,
                                                       const char *e) {
  lua_State *L = ms->L;
  strwork_ms(ms, SW_REPLACEMENT, 1);
  switch (lua_type(L, 3)) {
    case LUA_TNUMBER:
    case LUA_TSTRING: {
      add_s(ms, b, s, e);
  ...
  add_work_value(ms, b);  /* add result to accumulator */
}
...
static int str_gsub (lua_State *L) {
  ...
  strwork_ms(&ms, SW_OUTPUT, ms.src_end-src);
...
static const char *scanformat (lua_State *L, const char *strfrmt, char *form) {
  ...
  *(form++) = '%';
  strscpy(form, strfrmt, p - strfrmt + 1);
  form += p - strfrmt + 1;
  *form = '\0';
  return p;
}
